    ...
    guint * length, guint8 ** header, GstDPVersion version)
{
  ...
  *length = GST_DP_HEADER_LENGTH;
  ...
  /* version, flags, type */
  GST_DP_INIT_HEADER (((guint8 *)h), version, flags, GST_DP_PAYLOAD_BUFFER);

#if defined(__WINSCW__) || defined(__WINS__)
  
  /* buffer properties */
  GST_WRITE_UINT32_BE ((guint8 *)h + 6, GST_BUFFER_SIZE (buffer));
  GST_WRITE_UINT64_BE ((guint8 *)h + 10, GST_BUFFER_TIMESTAMP (buffer));
  GST_WRITE_UINT64_BE ((guint8 *)h + 18, GST_BUFFER_DURATION (buffer));
  GST_WRITE_UINT64_BE ((guint8 *)h + 26, GST_BUFFER_OFFSET (buffer));
  GST_WRITE_UINT64_BE ((guint8 *)h + 34, GST_BUFFER_OFFSET_END (buffer));
  GST_WRITE_UINT16_BE ((guint8 *)h + 42, GST_BUFFER_FLAGS (buffer) & flags_mask);

  GST_DP_SET_CRC ((guint8 *)h, flags, GST_BUFFER_DATA (buffer), GST_BUFFER_SIZE (buffer));
  ...
  *header = (guint8 *)h;
    ...
    tmp = (guint8 *)h ;
    tmp = tmp + 6;
    GST_WRITE_UINT32_BE ((guint32 *)tmp, GST_BUFFER_SIZE (buffer));
    tmp = tmp + 4;
    GST_WRITE_UINT64_BE ((guint64 *)tmp, GST_BUFFER_TIMESTAMP (buffer));
    tmp = tmp + 8;
    GST_WRITE_UINT64_BE ((guint64 *)tmp, GST_BUFFER_DURATION (buffer));
    tmp = tmp + 8;
    GST_WRITE_UINT64_BE (tmp, GST_BUFFER_OFFSET (buffer));
    tmp = tmp + 8;
    GST_WRITE_UINT64_BE (tmp, GST_BUFFER_OFFSET_END (buffer));
tmp = tmp + 8;
GST_WRITE_UINT16_BE (tmp, GST_BUFFER_FLAGS (buffer) & flags_mask);
    ...
    guint * length, guint8 ** header, guint8 ** payload, GstDPVersion version)
{
  ...
  *length = GST_DP_HEADER_LENGTH;
...
#else
  /* RAKHI version, flags, type */
  GST_DP_INIT_HEADER (((guint8 *)h), version, flags, GST_DP_PAYLOAD_CAPS);
  
  tmp = (guint8 *)h;
    /* buffer properties */
  tmp = tmp + 6; 
  /*   buffer properties */ 
  GST_WRITE_UINT32_BE (tmp, payload_length);
  tmp = tmp + 4;
  GST_WRITE_UINT64_BE (tmp, (guint64) 0);
  tmp = tmp + 8;
  GST_WRITE_UINT64_BE (tmp, (guint64) 0);
  tmp = tmp + 8;
  GST_WRITE_UINT64_BE (tmp, (guint64) 0);
  ...
  GST_WRITE_UINT64_BE (tmp, (guint64) 0);
#endif
  GST_DP_SET_CRC ((guint8 *)h, flags, string, payload_length);
  ...
  *header = h;
  ...
  /* now we can create and fill the header */
    ...
    *length = GST_DP_HEADER_LENGTH;
    ...
    /* version, flags, type */
    tmp = (guint8 *)h;
    GST_DP_INIT_HEADER (tmp, GST_DP_VERSION_0_2, flags,
        GST_DP_PAYLOAD_EVENT_NONE + GST_EVENT_TYPE (event));  
    tmp = tmp + 6;
    GST_WRITE_UINT32_BE (tmp, (guint32) pl_length);
    /* timestamp */
    tmp = tmp + 4;
    ...
    guint * length, guint8 ** header, guint8 ** payload)
{
  ...
  *header = h;
  ...
  /* version, flags, type */
  tmp = (guint8 *)h;
    GST_DP_INIT_HEADER (tmp, GST_DP_VERSION_1_0, flags,
        GST_DP_PAYLOAD_EVENT_NONE + GST_EVENT_TYPE (event));

    //tmp = tmp + 6;    
    /* length */
    GST_WRITE_UINT32_BE (tmp + 6, pl_length);
    /* timestamp */
    tmp = tmp + 10;
    GST_WRITE_UINT64_BE (tmp, GST_EVENT_TIMESTAMP (event));
...
#else
  tmp = (guint8 *)header + 10;
  GST_BUFFER_TIMESTAMP (buffer) = GST_READ_UINT64_BE (tmp);
  tmp = (guint8 *)header + 18;
  GST_BUFFER_DURATION (buffer) = GST_READ_UINT64_BE (tmp);
  tmp = (guint8 *)header + 26;
  GST_BUFFER_OFFSET (buffer) = GST_READ_UINT64_BE (tmp);
  tmp = (guint8 *)header + 34;
  GST_BUFFER_OFFSET_END (buffer) = GST_READ_UINT64_BE (tmp);
  tmp = (guint8 *)header + 42;
  GST_BUFFER_FLAGS (buffer) = GST_READ_UINT16_BE (tmp);
#endif  
  return buffer;
}
...
#else      
      tmp = (guint8 *)header + 10;
     // GST_EVENT_TIMESTAMP (event) = GST_READ_UINT64_BE (tmp);

	//Changes to avoid crashes on ARMv5				 
	GST_EVENT_TIMESTAMP (event) =  (((guint64) (((guint8 *) (tmp))[0])) << 56);
	GST_EVENT_TIMESTAMP (event) |= (((guint64) (((guint8 *) (tmp))[1])) << 48);
	GST_EVENT_TIMESTAMP (event) |= (((guint64) (((guint8 *) (tmp))[2])) << 40);
	GST_EVENT_TIMESTAMP (event) |= (((guint64) (((guint8 *) (tmp))[3])) << 32);
	GST_EVENT_TIMESTAMP (event) |= (((guint64) (((guint8 *) (tmp))[4])) << 24);
	GST_EVENT_TIMESTAMP (event) |= (((guint64) (((guint8 *) (tmp))[5])) << 16);	 
	GST_EVENT_TIMESTAMP (event) |= (((guint64) (((guint8 *) (tmp))[6])) << 8);
	GST_EVENT_TIMESTAMP (event) |= (((guint64) (((guint8 *) (tmp))[7])) << 0);				 
	
	//		GST_EVENT_TIMESTAMP (event) = GST_SECOND;
#endif      
      break;
    case GST_EVENT_SEEK:
    {
