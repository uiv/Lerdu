...
#ifdef __SYMBIAN32__
static void gst_app_src_base_init (gpointer g_class);
...
static GType object_type = 0;                      
 if (G_UNLIKELY (object_type == 0)) {                  
   object_type = gst_type_register_static_full(GST_TYPE_BASE_SRC,
       "GstAppSrc",
       sizeof (GstAppSrcClass),
       (GBaseInitFunc) gst_app_src_base_init,
       NULL,
       (GClassInitFunc) gst_app_src_class_init_trampoline,
       NULL,       
       NULL,
       sizeof (GstAppSrc),   
       0,
       (GInstanceInitFunc) gst_app_src_init,
       NULL,        
       (GTypeFlags) 0);
