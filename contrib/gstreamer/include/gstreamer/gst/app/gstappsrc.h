...
IMPORT_C GType gst_app_src_get_type(void);

IMPORT_C void             gst_app_src_set_caps         (GstAppSrc *appsrc, const GstCaps *caps);
IMPORT_C GstCaps*         gst_app_src_get_caps         (GstAppSrc *appsrc);

IMPORT_C void             gst_app_src_set_size         (GstAppSrc *appsrc, gint64 size);
IMPORT_C gint64           gst_app_src_get_size         (GstAppSrc *appsrc);

IMPORT_C void             gst_app_src_set_stream_type  (GstAppSrc *appsrc, GstAppStreamType type);
IMPORT_C GstAppStreamType gst_app_src_get_stream_type  (GstAppSrc *appsrc);

IMPORT_C void             gst_app_src_set_max_bytes    (GstAppSrc *appsrc, guint64 max);
IMPORT_C guint64          gst_app_src_get_max_bytes    (GstAppSrc *appsrc);

IMPORT_C void             gst_app_src_set_latency      (GstAppSrc *appsrc, guint64 min, guint64 max);
IMPORT_C void             gst_app_src_get_latency      (GstAppSrc *appsrc, guint64 *min, guint64 *max);

IMPORT_C void             gst_app_src_set_emit_signals (GstAppSrc *appsrc, gboolean emit);
IMPORT_C gboolean         gst_app_src_get_emit_signals (GstAppSrc *appsrc);

IMPORT_C GstFlowReturn    gst_app_src_push_buffer      (GstAppSrc *appsrc, GstBuffer *buffer);
IMPORT_C GstFlowReturn    gst_app_src_end_of_stream    (GstAppSrc *appsrc);

IMPORT_C void             gst_app_src_set_callbacks    (GstAppSrc * appsrc,
